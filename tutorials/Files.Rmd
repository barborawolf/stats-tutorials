...
**bw** stands here for **b**irth **w**eights. The data frame records the weights of newborn babies, along with several pieces of information about their mothers. To see the data in R, we can just type the name of the variable in the console, as we have seen before.

```{r}
bw
```

We will explore these data later, but first we will learn how to load the same data from other formats.
...
I will use csv files for most of the remaining tutorials that involve loading data.
...
If you are in doubt about the format of a delimited text file, open it first in a text editor outside of R to see what it contains. You can also see the contents of a text file from Rstudio, either by opening it via *File* -> *Open File*, or using the `file.show()` function, with the name of the file as input.
