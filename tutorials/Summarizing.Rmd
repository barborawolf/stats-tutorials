...
Each of these two functions can do a bit more, to produce a more detailed summary. `group_by()` can group by combinations of factor levels, just by adding the names of more grouping variables as further inputs. And `summarize()` can add more summary statistics if we add more `=` assignments as inputs.
...
That's all for dplyr for the moment, but we will learn more about it in future tutorials.
