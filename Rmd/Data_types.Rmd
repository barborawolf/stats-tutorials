...
Finally, we may occasionally want to organize more complex collections of data, in which the values are not all of the same class or length, but are still grouped together in order. Lists allow us to do this. They can be creaed much like a data frame, using the `list()` function, and allocating values to named entries within the list. Each value can be anything at all: a single number, a character string, a vector, a data frame, etc.
...
Alternatively, we can index a list by the order of the entries, as we do for a vector. However, if we do this, we do not get the contents of that entry in the list. Instead, we get a new list that just has one entry: the entry we asked for.
...
class(survey[[2]]) # (it's the character vector stored in the list)
```
